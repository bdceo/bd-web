...
import static com.bdsoft.wlqp.Server.*;
	...
	public void run() {
		while (flag) {
			try {
				...
				if (msg.startsWith("<#PLAY#>")) { // 客户端发牌并且告知服务器
					// 并且服务器判断下一个出牌的玩家
					...
					currPlayer = next;
				} else if (msg.startsWith("<#COUNT#>")) {// 转发<#COUNT#>
					// 发送要出的牌的牌号和当前玩家的标志位
					player1.dout.writeUTF(msg);
					player2.dout.writeUTF(msg);
					player3.dout.writeUTF(msg);
				} else if (msg.startsWith("<#I_WIN#>")) {// 收到<#I_WIN#>(赢的)消息
					// 并发送<#FINISH#>消息
					int currNumTemp = -1;
					if (this == player1) {
						currNumTemp = 1;
					...
					player3.din.close();
					player3.sc.close();
				} else if (msg.startsWith("<#NO_PLAY#>")) {// 客户端点击放弃按钮之后
					// 服务器端判断下一个玩家是谁
					ServerAgent next = null;
					if (currPlayer == player1) {
						next = player2;
					...
					currPlayer = next;
				} else if (msg.startsWith("<#EXIT#>")) {// 收到客户端退出消息 并进行相关的设置
					Server.count = 0;
